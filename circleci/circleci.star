...
def logo_for_status(status):
    if status == "success":
        return CIRCLECI_LOGO_GREEN
    if status == "failed":
        return CIRCLECI_LOGO_RED
    if status == "running":
        return CIRCLECI_LOGO_YELLOW
